...
// Store Employees Stat Table --START
    ...
    (async function(){
        try{
            connection = await oracledb.getConnection(connectDB.cred);
            console.log("Axios request received with request body:",req.body[0].store_sid);
            await connection.execute(`SELECT b.employee1_sid as EMP_SID,b.employee1_full_name AS EMPLOYEE,COUNT(DISTINCT(a.sid))AS DOC_COUNT,SUM(DECODE(b.item_type,2,b.qty*-1,b.qty))AS Doc_Qty,ROUND(SUM(DECODE(b.item_type,2,b.qty*-1,b.qty)*(b.price)),2)AS Sale_Total,ROUND((ROUND(SUM(DECODE(b.item_type,2,b.qty*-1,b.qty)*(b.price)),2)/COUNT(DISTINCT(a.sid))),2) AS AVG_BKT,ROUND(SUM(DECODE(b.item_type,2,b.qty*-1,b.qty))/COUNT(DISTINCT(a.sid)),2) AS UPT,ROUND(SUM(ROUND((((CASE WHEN B.ITEM_TYPE=2 THEN B.QTY*-1 ELSE B.QTY END)*(B.ORIG_PRICE))-((CASE WHEN B.ITEM_TYPE=2 THEN B.QTY*-1 ELSE B.QTY END)*(B.PRICE))),2)),2) AS "DISC AMT", ROUND(SUM(DECODE(b.item_type,2,b.qty*-1,b.qty)*(b.TAX_AMT)),2) AS Tax_Total FROM rps.document a join rps.document_item b on b.doc_sid = a.sid LEFT join rps.document_disc c on c.doc_sid = a.sid WHERE a.created_datetime BETWEEN to_date(:id1,'DD.MM.YYYY:HH24:MI') AND to_date(:id2,'DD.MM.YYYY:HH24:MI') and a.receipt_type in (0,1)and b.item_type in (1,2) and a.status =4 AND a.store_sid=:id3 GROUP BY b.employee1_full_name,b.employee1_sid`,[req.body[0].date1Par,req.body[0].date2Par,req.body[0].store_sid],{
                fetchInfo : { 
...
// Store Tender Stat Table --START
    ...
    (async function(){
        try{
            connection = await oracledb.getConnection(connectDB.cred);
            console.log("Axios request received with request body:",req.body[0].store_sid);
            await connection.execute(`SELECT DECODE(A.TENDER_TYPE,6,'Split',DECODE(A.TENDER_NAME,NULL,'Credit Card',A.TENDER_NAME)) AS TENDER,ROUND(SUM(ROUND(((CASE WHEN B.ITEM_TYPE=2 THEN B.QTY*-1 ELSE B.QTY END)*(B.PRICE)), 2)),2) AS "EXT PRICE W TAX" FROM RPS.DOCUMENT A INNER JOIN RPS.DOCUMENT_ITEM B ON B.DOC_SID = A.SID WHERE A.RECEIPT_TYPE IN(0, 1) AND B.ITEM_TYPE IN(1, 2) AND A.STATUS=4 AND a.created_datetime BETWEEN to_date(:id1,'DD.MM.YYYY:HH24:MI') AND to_date(:id2,'DD.MM.YYYY:HH24:MI') AND a.store_sid=:id3 GROUP BY A.STORE_NAME,DECODE(A.TENDER_TYPE,6,'Split',DECODE(A.TENDER_NAME, NULL,'Credit Card',A.TENDER_NAME))`,[req.body[0].date1Par,req.body[0].date2Par,req.body[0].store_sid],{
                fetchInfo : { 
                    "EMP_SID" : {type:oracledb.STRING},
